...
function PlacesPage() {
  ...
  const [price, setPrice] = useState('') // price per night
  ...
  async function savePlace(e) {
    ...
    if (id) {
      ...
      const res = await axios.put(`${SERVER_URL}/api/places`, data)
    } else {
  ...
  // function used to select the main photo out of all uploaded photos by clicking the star icon
  function setAsMainPhoto(e, photo) {
    e.preventDefault()
    // changing photo order
    setAddedPhotos((prev) => [photo, ...prev.filter((file) => file != photo)])
  }
      ...
      {action === 'new' && (
        <div className="">
          <form onSubmit={savePlace}>
              ...
              {addedPhotos.length > 0 &&
                addedPhotos.map((link) => (
                  <div key={link} className="h-32 flex relative">
                    ...
                    {/* adding start icon button to select main photo */}
                    <button
                      onClick={(e) => setAsMainPhoto(e, link)}
                      className="absolute bottom-1 left-1 cursor-pointer bg-black p-1 bg-opacity-50 rounded-xl"
                    >
                      {link == addedPhotos[0] && (
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="mt-4 grid sm:grid-cols-2 md:grid-cols-4 gap-2 mb-2">
              <div>
